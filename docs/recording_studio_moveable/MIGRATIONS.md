...
```bash
rails generate recording_studio_moveable:migrations
```
...
```bash
rails generate recording_studio_moveable:migrations --pretend
```
...
```bash
rails generate recording_studio_moveable:migrations --no-skip-existing --force
```
...
```bash
# From the gem root
touch db/migrate/$(date +%Y%m%d%H%M%S)_create_recording_studio_moveable_widgets.rb
```
...
1. **Use UUID primary keys** for PostgreSQL compatibility:
   ...
   ```ruby
   class CreateRecordingStudioMoveableWidgets < ActiveRecord::Migration[7.1]
     def change
       create_table :recording_studio_moveable_widgets, id: :uuid do |t|
...
2. **Prefix table names** with the gem name to avoid conflicts:
   ...
   ```ruby
   # Good: recording_studio_moveable_widgets
   # Bad: widgets
   ```
...
4. **Add indexes** for frequently queried columns:
   ...
   ```ruby
   add_index :recording_studio_moveable_widgets, :name
   add_index :recording_studio_moveable_widgets, :created_at
   ```
...
db/
└── migrate/
    ├── 20250101000001_create_recording_studio_moveable_examples.rb
    └── 20250101000002_create_recording_studio_moveable_widgets.rb
```
...
- Class names change (e.g., `CreateRecordingStudioMoveableExamples` → `CreateMyEngineExamples`)
- Table names change (e.g., `recording_studio_moveable_examples` → `my_engine_examples`)
