...
RecordingStudioMoveable provides a powerful hook system that allows host applications to customize engine behavior without modifying the engine's source code.
...
```ruby
# config/initializers/recording_studio_moveable.rb
RecordingStudioMoveable.configure do |config|
  config.hooks.after_initialize do
    Rails.logger.info "RecordingStudioMoveable initialized!"
  end
...
```ruby
# app/hooks/recording_studio_moveable_audit_hook.rb
class RecordingStudioMoveableAuditHook
...
# config/initializers/recording_studio_moveable.rb
RecordingStudioMoveable.configure do |config|
  config.hooks.after_service RecordingStudioMoveableAuditHook.new
end
```
...
RecordingStudioMoveable.configure do |config|
  config.hooks.before_service do |service_class, args|
    Rails.logger.info "[RecordingStudioMoveable] #{service_class.name} called"
  end
  
  config.hooks.after_service do |service_class, result|
    status = result.success? ? "succeeded" : "failed"
    Rails.logger.info "[RecordingStudioMoveable] #{service_class.name} #{status}"
  end
end
```
...
RecordingStudioMoveable.configure do |config|
  config.hooks.around_service do |service, block|
    ...
    StatsD.measure("recording_studio_moveable.#{service.class.name.demodulize.underscore}", duration)
    result
  end
end
```
...
<% content_for :recording_studio_moveable_header do %>
  <div class="custom-header">Your custom header</div>
<% end %>

<% content_for :recording_studio_moveable_footer do %>
...
```
app/views/recording_studio_moveable/home/_sidebar.html.erb  # Overrides engine partial
```
