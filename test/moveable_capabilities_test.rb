...
class MoveableCapabilitiesTest < Minitest::Test
  ...
  def test_capability_type_names_flattens_stringifies_and_deduplicates_values
    capabilities = RecordingStudio::Moveable::Capabilities::Moveable

    type_names = capabilities.capability_type_names([[String, "Folder"], nil, String, :Workspace])

    assert_equal %w[String Folder Workspace], type_names
  end
  ...
  def test_apply_capability_registers_capability_and_options
    ...
    RecordingStudio.stub(:enable_capability, ->(*args, **kwargs) { enabled << [args, kwargs] }) do
      RecordingStudio.stub(:set_capability_options, ->(*args, **kwargs) { configured << [args, kwargs] }) do
        capabilities.apply_capability(base, %w[Workspace Folder], allow_cross_root: true)
      end
    end
  ...
  def test_build_capability_module_applies_capability_when_included
    ...
    mod = capabilities.build_capability_module(type_names, options)
    klass = Class.new do
      def self.name
        "ExampleType"
      end
    ...
    capabilities.stub(:apply_capability, lambda { |base, received_type_names, received_options|
      applied = [base.name, received_type_names, received_options]
    }) do
      klass.include(mod)
    end

    assert_equal ["ExampleType", type_names, options], applied
  end

  def test_to_normalizes_types_and_options_before_building_module
    capabilities = RecordingStudio::Moveable::Capabilities::Moveable
    captured = nil

    capabilities.stub(:build_capability_module, lambda { |type_names, options|
      captured = [type_names, options]
      :built_module
    }) do
      result = capabilities.to(String, "Folder", nil, allow_cross_root: true)

      assert_equal :built_module, result
    end

    assert_equal [%w[String Folder], { allow_cross_root: true }], captured
  end

  def test_legacy_movable_alias_delegates_to_moveable_capability_builder
    moveable = RecordingStudio::Moveable::Capabilities::Moveable
    delegated = nil

    moveable.stub(:to, lambda { |*args|
      delegated = args
      :legacy_module
    }) do
      result = RecordingStudio::Capabilities::Movable.to("Workspace", allow_cross_root: true)

      assert_equal :legacy_module, result
    end

    assert_equal ["Workspace", { allow_cross_root: true }], delegated
  end

  def test_recording_methods_expose_allowed_parent_types_and_cross_root_flag
    ...
    capability_options = lambda do |_capability, for_type:|
      ...
      { allowed_parent_types: [:Workspace, "RecordingStudioFolder"], allow_cross_root: true }
    end

    RecordingStudio.stub(:capability_options, capability_options) do
      assert_equal %w[Workspace RecordingStudioFolder], recording.send(:moveable_allowed_parent_types)
